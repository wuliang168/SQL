...
(select COUNT(EID) from Workrecord where EID=a.EID and DATEDIFF(mm,tianxiedate,GETDATE())=1 and id in (select min(id) from Workrecord where DATEDIFF(mm,tianxiedate,GETDATE())=1 group by EID,CONVERT(VARCHAR(10),tianxiedate,110))) as tianxieday, a.Name as Name,a.DepID as depid1,
