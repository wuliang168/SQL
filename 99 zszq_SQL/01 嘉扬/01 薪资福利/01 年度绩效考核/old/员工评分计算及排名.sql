...
from pscore as a,pscore as b,pscore as c,pscore as d
where a.SCORE_EID=@SCORE_Eid AND a.SCORE_STATUS=3 AND a.SCORE_TYPE in (25,6,7) AND isnull(a.compliance,0)=15 
AND a.EID=b.EID AND b.SCORE_STATUS=1
AND a.EID=c.EID AND c.SCORE_STATUS=2
AND a.EID=d.EID AND d.SCORE_STATUS=7 AND isnull(d.SCORE_STATUS,0)=7 AND ISNULL(d.SUBMIT,0)=1
update a
set a.scoretotal=isnull(a.score9,0)*isnull(a.weight,0)/100+isnull(b.score8,0)+isnull(c.score9,0)*isnull(c.weight,0)/100
from pscore as a,pscore as b,pscore as c
where a.SCORE_EID=@SCORE_Eid AND a.SCORE_STATUS=3 AND a.SCORE_TYPE in (25,6,7) AND isnull(a.compliance,0)=0 
AND a.EID=b.EID AND b.SCORE_STATUS=1
AND a.EID=c.EID AND c.SCORE_STATUS=2
-- 刷新递交状态
update c
