...
GO
ALTER  Procedure [dbo].[eSP_pDepPensionPerMMBSStartH]
-- skydatarefresh eSP_pDepPensionPerMMBSStartH
    @leftid varchar(20),
    @EID int,
    @RetVal int=0 Output
As
...
Begin
    ...
    declare @pProcessID int,@SalaryPayID int
    set @pProcessID=convert(int,SUBSTRING(@leftid,0,CHARINDEX('-',@leftid)))
    ...
    where a.SalaryPayID=@SalaryPayID and (@SalaryPayID in (1,2,3,10,11,12,13,14,15,16,19) or @SalaryPayID=7 or @SalaryPayID in (4,5))
    ...
    AND ((b.SalaryPayID in (1,2,3,10,11,12,13,14,15,16,19) and @SalaryPayID=1) or (b.SalaryPayID in (4,5) and @SalaryPayID=4) or (b.SalaryPayID=7 and @SalaryPayID=7))
    -- 异常流程
    If @@Error<>0
    Goto ErrM
