...
GO
ALTER proc [dbo].[CSP_HeadPrize_import_AF]--CSP_HeadPrize_import_AF()
    @URID int,
    @RetVal int=0 output
AS
/*
...
Begin
    ...
    -- 拷贝pHeadPrize_import的奖金到pEmpEmoluBonusAdd
    ...
    a.BonusType,a.BonusAmount,(select DepID from oDepartment where DepAbbr=a.BonusDepID),
    ...
    from pHeadPrize_import a
    where a.HeadContact=(select EID from SkySecUser where ID=@URID)
    -- 异常流程
	IF @@Error <> 0
	Goto ErrM


    -- 清除导入员工奖金pHeadPrize_import
    delete from pHeadPrize_import where HeadContact=(select EID from SkySecUser where ID=@URID)
    -- 异常流程
	IF @@Error <> 0
	Goto ErrM
