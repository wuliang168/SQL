...
GO
ALTER proc [dbo].[pSP_pYear_ScoreEachL]
    @id int,
    @URID int,
    @RetVal int=0 OutPut
as
...
Begin
    ...
    -- 互评和胜任素质评测已开启，无法重新开启！
    IF Exists(select 1 from pYear_ScoreEachL a where ISNULL(a.Initialized,0)=1)
    Begin
        Set @RetVal=1001090
        Return @RetVal
    End
    ...
    and a.EID not in (select EID from pYear_Score where Score_Status=1 and Score_Type1 in (1,2,10,30,31,32))
    and a.EID in (select EID from pYear_Score where Score_Status=99 and Score_Type1 in (1,2,10,30,31,32))
    -- 异常处理
    IF @@Error <> 0
    Goto ErrM
    ...
    from pYear_Score a
    where a.SCORE_STATUS=1 and a.Score_Type1 in (1,2,10,30,31,32)
    -- 异常处理
    if @@ERROR<>0
    GOTO ErrM
