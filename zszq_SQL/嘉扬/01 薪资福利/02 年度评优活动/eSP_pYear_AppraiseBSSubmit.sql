...
Begin
    ...
    ---- 相同的团队
    IF Exists(select 1 from pYear_Appraise where AppraiseEID=@AppraiseEID and AppraiseDepID=@AppraiseDepID and DepID is not NULL
    ...
    ---- 相同的个人
    ------ 后台
    IF Exists(select 1 from pYear_Appraise where AppraiseEID=@AppraiseEID and AppraiseDepID=@AppraiseDepID and EID is not NULL
    ...
    End
    ------ 前台
    IF Exists(select 1 from pYear_Appraise where AppraiseEID=@AppraiseEID and AppraiseDepID=@AppraiseDepID and Identification is not NULL
    ...
    ---- 相同的团队
    IF Exists(select 1 from pYear_Appraise where AppraiseEID=@AppraiseEID and AppraiseDepID=@AppraiseDepID and Reason is NULL)
    Begin
        Set @RetVal=1003106
        Return @RetVal
    End
    ...
    -- 更新年度评优部门负责人递交时间
    update a
    set a.Submit=1,a.SubmitBy=(select ID from SkySecUser where EID=@AppraiseEID),a.SubmitTime=GETDATE()
    from pYear_Appraise a,pYear_AppraiseProcess b
    where a.AppraiseEID=@AppraiseEID and AppraiseDepID=@AppraiseDepID
    ...
    where a.AppraiseEID=@AppraiseEID and AppraiseDepID=@AppraiseDepID AND ISNULL(a.IsSubmit,0)=0 
    AND a.pYear_ID=b.ID and ISNULL(b.Submit,0)=1 and ISNULL(b.Closed,0)=0
    -- 异常处理
    IF @@Error <> 0
    Goto ErrM
