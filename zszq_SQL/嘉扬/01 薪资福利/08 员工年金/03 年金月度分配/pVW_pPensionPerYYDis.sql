...
else 0 end) as EmpPensionPerYYRST_new,b.EmpPensionPerMMTT as EmpPensionPerMM
from pPensionUpdatePerEmp a
inner join (select EID,BID,(ISNULL(EmpPensionPerMMATax,0)+ISNULL(EmpPensionPerMMBTax,0)) as EmpPensionPerMMTT from pEmpPensionPerMM_register) as b on ISNULL(b.EID,b.BID)=ISNULL(a.EID,a.BID)
left join (select m.EID,m.BID,m.PensionYear,
(select SUM(ISNULL(EmpPensionPerYYRST,0)) from pPensionUpdatePerEmp where DATEDIFF(YY,PensionYear,m.PensionYear)>=0 and ISNULL(EID,BID)=ISNULL(m.EID,m.BID)) as EmpPensionPerYYDisAccu
from pPensionUpdatePerEmp m
where m.PensionYear in (select PensionYear from pPensionPerYY where ISNULL(IsDisMM,0)=1)
