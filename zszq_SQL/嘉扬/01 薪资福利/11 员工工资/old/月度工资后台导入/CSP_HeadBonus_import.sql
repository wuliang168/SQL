...
GO
ALTER proc [dbo].[CSP_HeadBonus_import]--CSP_HeadBonus_import()
    @URID int,
    @Badge varchar(10),
    @Name nvarchar(50),
...
Begin
    ...
    insert into pHeadBonus_import (HeadContact,Badge,GeneralBonus,OneTimeAnnualBonus,Remark)
    select (select EID from SkySecUser where ID=@URID),@Badge,@GeneralBonus,@OneTimeAnnualBonus,@Remark
    -- 异常流程
	IF @@Error <> 0
	Goto ErrM
