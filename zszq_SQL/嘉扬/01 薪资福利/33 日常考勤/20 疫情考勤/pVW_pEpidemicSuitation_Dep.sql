...
(case when (select COUNT(Name) from pEpidemicSuitation where ReportTo=b.approverID and DATEDIFF(DD,GETDATE(),ESDATE)=0)>0 and b.SubmitType=1 then 1 when b.SubmitType=3 then 2 else 0 end) as SubmitStatus,
