...
GO
ALTER VIEW [dbo].[Attence_EachMonth]
AS
...
CONVERT(DECIMAL(5,1),COUNT(b.EID*1.0)+COUNT(c.EID)*1.0/2.0+COUNT(e.EID)*1.0/2.0-COUNT(g.EID)*1.0/2.0-COUNT(h.EID)*1.0/2.0) as WorkDays,
---- 未出勤天数=未打卡(未确认)(YCADays)/2
CONVERT(DECIMAL(5,1),COUNT(f.EID)*1.0/2.0) as AbnormWorkDays,
---- 迟到次数(未确认)=迟到(未确认)(YCLateDays)
COUNT(g.EID) as LateWorkDays,
---- 早退次数(未确认)=早退(未确认)(YCEarlyDays)
COUNT(h.EID) as EarlyWorkDays,
---- 比较日期
z.term
from eEmployee a
---- 距离今天2个月内
inner join lCalendar as z on DATEDIFF(mm,z.Term,GETDATE())<=2 and DATEDIFF(mm,z.Term,GETDATE())>0
---- 上午下午均打卡 DKFDays
...
---- 上午或下午打卡 DKHDays
left join (select EID,term as DKHDays from BS_DK_time where ((beginTime is NULL and endTime is not NULL) or (endTime is NULL and beginTime is not NULL))) c on c.eid=a.EID and Datediff(mm,c.DKHDays,z.Term)=0
---- 上午和下午均未打卡 DKFNDays
left join (select EID,term as DKFNDays from BS_DK_time where (beginTime is NULL and endTime is NULL)) d on d.eid=a.EID and Datediff(mm,d.DKFNDays,z.Term)=0
---- 未打卡(确认) YCACDays
