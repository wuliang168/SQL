...
GO
ALTER  Procedure [dbo].[eSP_EmpDepCalcUpdate]
-- skydatarefresh eSP_EmpDepCalcUpdate
    @RetVal int=0 Output
As
/*
...
Begin
    ...
    -- 月度表单中未出现员工，更新后自动添加
    insert into pEmpDepStat(MonthStat,DepType,DepID,EMPSum,GenderMSum,GenderFSum,AVGAGESum,HighLev_ASSTTSum,HighLev_UGTTSum,HighLev_MSTTSum,HighLev_DTTTSum,
    ...
    select @calcdate,a.DepType,a.DepID,a.EMPSum,a.GenderMSum,a.GenderFSum,a.AVGAGESum,a.HighLev_ASSTTSum,a.HighLev_UGTTSum,a.HighLev_MSTTSum,a.HighLev_DTTTSum,
    ...
    from (select a.DepType,a.DepID,SUM(a.EMP) as EMPSum,SUM(a.GenderM) as GenderMSum,SUM(a.GenderF) as GenderFSum,SUM(a.EMP*a.AVGAGE)/SUM(a.EMP) as AVGAGESum,
    ...
    GROUP BY DepType, DepID) b on a.DepType=b.DepType and a.DepID=b.DepID
    where 0 not in (select distinct DATEDIFF(MM,@calcdate,MonthStat) from pEmpDepStat)
    -- 异常流程
    If @@Error<>0
    Goto ErrM
