...
c.Address as Address,(select Title from eCD_Place where ID=c.Place) as Place,(select Title from eCD_City where ID=a.WorkCity) as WorkCity, 
...
(select Title from eCD_EduType where ID=c.HighLevel) as HighLevel, (select Title from eCD_DegreeType where ID=c.HighDegree) as HighDegree,c.schoolname as SchoolName, 
...
INNER JOIN dbo.pEMPAdminIDMD AS f ON a.EID=f.EID
