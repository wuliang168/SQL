...
Begin
    ...
    -- 申明邮件发送内容
    declare @LMonth smalldatetime,@TMonth smalldatetime,@id int
    ...
    End
    ---- @id
    set @id=(select ID from PPROCESS_MONTH where DATEDIFF(mm,kpimonth,@TMonth)=0)

    ---- 开启月度考核
    exec PSP_MONTHKQ @id,1
    ...
    +'<p>'+N'二、工作日志'+'<br></p>'
    +'<p>'+N'1、人资部定期对于合格率较低的部门（营业部）和个人进行通报；'+'<br></p>'
    +'<p>'+N'2、按规定全月累计未填写超过5天判定不合格，各部门（营业部）不合格率及不合格人员可在HR系统绩效查询菜单中查看；'+'<br></p>'
    +'<p>'+N'3、各级领导可以在“绩效查询”菜单中查看下属工作日志。'+'<br></p>'
    +'<p>'+N'4、各部门（营业部）管理员可在管理员菜单-日常事务-日志统计中查看并督促大家及时完成。'+'<br></p>'
    +'<p>'+N'工作日志操作说明详见链接 <a href="url">http://10.51.190.162/bs/help/workrecord.pdf</a>'+'<br></p>'
    +'<p>'+N'三、考勤（截止'+CONVERT(varchar(4),YEAR(@TMonth))+N'年'+CONVERT(varchar(2),MONTH(@TMonth))+N'月15日）'+'<br></p>'
    +'<p>'+N'1、总部人员请根据本人的异常出勤记录，在本周内填写异常说明或补充外出登记并提交领导审核确认，'+CONVERT(varchar(4),YEAR(@TMonth))+N'年'+CONVERT(varchar(2),MONTH(@TMonth))+N'月15日下班封存数据。'+'<br></p>'
    +'<p>'+N'2、营业部的日常考勤参照总部执行，具体以营业部规定为准。'+'<br></p>'
    +'<p>'+N'有任何疑问或操作问题请及时与本部门（营业部）绩效管理员（综合人事管理员）或人力资源部吴亮联系。'+'<br></p>'
