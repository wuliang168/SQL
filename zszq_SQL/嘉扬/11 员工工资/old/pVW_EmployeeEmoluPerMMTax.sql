...
(dbo.eFN_getPersonalIncomeTax(
    ...
    +ISNULL(a.FundEmpOver,0)+ISNULL(a.FundGRPOver,0)+ISNULL(a.BenefitsInKind,0))*0.3,2) 
    then ROUND((ISNULL(a.TotalPayAmount,0)-ISNULL(a.FundInsEMPTotal,0)-ISNULL(a.FundInsEMPPlusTotal,0)-ISNULL(a.PensionEMPBT,0)
    +ISNULL(a.FundEmpOver,0)+ISNULL(a.FundGRPOver,0)+ISNULL(a.BenefitsInKind,0))*0.7,2)
    ...
    +ISNULL(a.FundEmpOver,0)+ISNULL(a.FundGRPOver,0)+ISNULL(a.BenefitsInKind,0)
    end)-
    -- 本国和非本国员工
    (select TaxBase from oCD_TaxBaseType where ID=(case when l.country=41 then 1 else 2 end)))) as PersonalIncomeTax,
-- 一次性奖金税
...
then dbo.eFN_getPersonalIncomeTax(ROUND(ISNULL(a.OneTimeAnnualBonus,0)/12,2))
...
+ISNULL(a.FundEmpOver,0)+ISNULL(a.FundGRPOver,0)+ISNULL(a.BenefitsInKind,0))),2)) 
