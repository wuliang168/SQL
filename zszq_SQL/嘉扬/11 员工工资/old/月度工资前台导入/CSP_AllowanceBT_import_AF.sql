...
GO
ALTER proc [dbo].[CSP_AllowanceBT_import_AF]--CSP_AllowanceBT_import_AF()
    @URID int,
    @leftid int,
    @RetVal int=0 output
AS
...
Begin
    ...
    -- 拷贝pAllowanceBT_import的税前补贴到pEmployeeEmoluAllowanceBT
    update b
    ...
    from pAllowanceBT_import a,pEmployeeEmoluAllowanceBT b
    ...
    -- 清除导入员工税前补贴pAllowanceBT_import
    delete from pAllowanceBT_import where SalaryPayID=@leftid and SalaryContact=(select EID from SkySecUser where ID=@URID)
    -- 异常流程
	IF @@Error <> 0
	Goto ErrM
