...
GO
ALTER proc [dbo].[CSP_Comm_import_AF]--CSP_Comm_import_AF()
    @URID int,
    @leftid int,
    @RetVal int=0 output
AS
...
Begin
    ...
    -- 拷贝pComm_import的通讯费到pEmployeeEmolu
    update b
    ...
    from pComm_import a,pEmployeeEmolu b
    ...
    -- 清除导入员工通讯费pComm_import
    delete from pComm_import where SalaryPayID=@leftid and SalaryContact=(select EID from SkySecUser where ID=@URID)
    -- 异常流程
	IF @@Error <> 0
	Goto ErrM
