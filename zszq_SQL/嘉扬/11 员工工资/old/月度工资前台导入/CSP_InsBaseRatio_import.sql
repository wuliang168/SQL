...
GO
ALTER proc [dbo].[CSP_InsBaseRatio_import]--CSP_InsBaseRatio_import()
    @URID int,
    @leftid int,
    @Badge varchar(10),
    @Name nvarchar(50),
...
Begin
    ...
    -- 将导入的文件插入到pInsBaseRatio_import表项中
    insert into pInsBaseRatio_import (SalaryPayID,SalaryContact,Badge,InsuranceBase,InsuranceLoc,
    EndowInsRatioEMP,EndowInsRatioGRP,MedicalInsRatioEMP,MedicalInsRatioGRP,UnemployInsRatioEMP,UnemployInsRatioGRP,
    MaternityInsRatioGRP,InjuryInsRatioGRP)
    select @leftid,(select EID from SkySecUser where ID=@URID),@Badge,@InsuranceBase,@InsuranceLoc,@EndowInsRatioEMP,
    @EndowInsRatioGRP,@MedicalInsRatioEMP,@MedicalInsRatioGRP,@UnemployInsRatioEMP,@UnemployInsRatioGRP,@MaternityInsRatioGRP,
    @InjuryInsRatioGRP
    -- 异常流程
	IF @@Error <> 0
	Goto ErrM
