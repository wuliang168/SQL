...
GO
ALTER proc [dbo].[CSP_HeadAllowanceBT_import]--CSP_HeadAllowanceBT_import()
    @URID int,
    @Badge varchar(10),
    @Name nvarchar(50),
...
Begin
    ...
    -- 将导入的文件插入到pHeadAllowanceBT_import表项中
    insert into pHeadAllowanceBT_import (HeadContact,Badge,SponsorAllowanceBT,DirverAllowanceBT,
    ComplAllowanceBT,RegFinMGAllowanceBT,SalesMGAllowanceBT,CommAllowanceBT,CommAllowancePlus,Remark)
    select (select EID from SkySecUser where ID=@URID),@Badge,@SponsorAllowanceBT,@DirverAllowanceBT,
    @ComplAllowanceBT,@RegFinMGAllowanceBT,@SalesMGAllowanceBT,@CommAllowanceBT,@CommAllowancePlus,@Remark
    -- 异常流程
	IF @@Error <> 0
	Goto ErrM
