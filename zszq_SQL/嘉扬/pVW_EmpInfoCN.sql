...
(select Title from ECD_EMPSTATUS where ID=a.Status) as 在职状态, (select Title from eCD_City where ID=a.WorkCity) as 工作地, 
...
(select Title from eCD_EduType where ID=c.HighLevel) as 最高学历, (select Title from eCD_DegreeType where ID=c.HighDegree) as 最高学位, 
